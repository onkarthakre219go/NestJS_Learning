...
export class StudentController {
   ...
   /*  @Get()
    ...
    @Put(':id')
    updateOneStudent(@Param('id') id: string, @Body() data: { name?: string; age?: number }) {
        return this.studentService.updateStudent(Number(id), data);
    }

    @Patch(':id')
    patchOneStudent(@Param('id') id: string, @Body() data: Partial<{ name: string; age: number }>) {
        return this.studentService.patchStudent(Number(id), data);
    }

    @Delete(':id')
    deleteOneStudent(@Param('id') id: string) {
